...
To ensure no pain at revision, each figure panel visualization script needs to be fully independent from the others, and must read and write from the shared `data` and `results` folders only. To avoid pain, each **small** analysis task should require a separate branch, to be merged with master and removed at completion.
